...
class TrainingDataset(Dataset):
    def __init__(self, data, feature, mask=None):
        self.data = data
    ...
    def __getitem__(self, index):
        if self.mask is None:
            ...
            return self.data[index], self.feature[index], self.mask[index]
...
def train_eval(config):
    ...
    train_dataset = TrainingDataset(data=train_data, feature=user_feat[train_user_index], mask=train_mask)
    ...
    for epoch in range(epochs):
        ...
        for x in train_dataloader:
            ...
            x_hat, mean, log_var, prior_mean, prior_log_var = model(x, w)
