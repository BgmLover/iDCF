...
def train_eval(config):
    ...
    model = MFMF(num_users=n_users, num_items=n_items,
                 feature_dim=data_params["user_feature_dim"],
    ...
    for epoch in range(config["epochs"]):
        ...
        for index, (uid, iid, rating) in enumerate(train_loader):
            ...
            predict = model(uid, iid, u_feat, sample=True).view(-1)
        ...
        model.eval()
        validation_performance = evaluate(metric, val_loader, model, user_feat, device=DEVICE, params=evaluation_params)
    ...
    if args.tune:
        ...
        if args.test_seed:
            ...
            if args.data_params["name"] == "coat":
                ...
                lr = 5e-5
                wd = 1e-6
            elif args.data_params["name"] == "kuai":
                lr = 5e-5
                wd = 1e-6
